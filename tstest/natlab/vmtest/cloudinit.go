...
// tailscaled launch entries. mode is validated in AddNode.
func writeLinuxDNSModeSetup(ud *strings.Builder, mode DNSMode) {
	switch mode {
	case DNSDefault:
		// Leave the image's DNS config alone: systemd-resolved stays enabled,
		// so tailscaled selects it (or whatever the image runs by default).
	case DNSDirect:
